...
concept ControlLaw =
    requires(C law,
             const typename C::Measurement& m)
{
    ...
    { law.Initialize() }
        -> std::same_as<std::expected<typename C::State, std::string>>;

    { law.Compute(m) }
        -> std::same_as<std::expected<
            std::pair<typename C::Command, typename C::State>,
            std::string>>;
};
    ...
    requires(FB fb)
{
    ...
    { fb.Read() }
        -> std::same_as<std::expected<
            std::pair<typename Law::Measurement, typename FB::State>,
            std::string>>;
};
...
concept Actuator =
    ControlLaw<Law> &&
    requires(AC ac, const typename Law::Command& cmd)
{
    typename AC::State;
    ...
    { ac.Write(cmd) } 
        -> std::convertible_to<std::expected<typename AC::State, std::string>>;
};
...
public:
    ...
    Controller(FB& feedback, ACT& actuator, Law& law)
        : fb_(feedback), act_(actuator), law_(law)
    {}
...
struct std::formatter<ControllerState<FB, LAW, ACT>> {
    ...
    auto format(const ControllerState<FB, LAW, ACT>& s, FormatContext& ctx) const {
        return std::format_to(
            ...
            to_string(s.feedback_state),
            to_string(s.control_state),
